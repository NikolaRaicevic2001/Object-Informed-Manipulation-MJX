...
uv run python -m oim.run_eval --group-by object task --ablate samples horizon n_admm rho consensus_object_weight temperature iterations
...
| `--trajectory-plot` | algorithm × task grid of measured object paths (rows are methods, columns are scenes): trial digit per mark (Roman for the second goal orientation), colour is time, obstacles yellow, starts green, each goal orientation its own shade of red with its position tolerance as a dashed circle and its orientation tolerance as the outline repeated at either limit (the legend carries both numbers) |
