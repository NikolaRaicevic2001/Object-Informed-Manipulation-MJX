...
| **Residuals are an RMS, not a norm** | `residual_norm` is handed both blocks' residuals concatenated over the horizon, a $(2H, \dim)$ array, so a plain 2-norm grew like $\sqrt{2H\dim}$ — scale-free in wrench units but *not* in the horizon, which silently tightened $\epsilon_r, \epsilon_s$ by 1.15× on an $H$ 24 → 32 change, and with them the early exit. Dividing by $\sqrt{\text{size}}$ makes the value read as "typical disagreement per channel, as a fraction of $D^{-1}$". $\epsilon_r, \epsilon_s$ moved 0.5 → 0.05 with it; measured primal on `open_table` is 0.57, so the exit still never fires and behaviour is unchanged. |
...
| **`twist` inverts a plant `step` no longer integrates** | `"twist"` is the exact inverse of eq. 4, $\dot{x}^o = D w^o$ — but `step` has integrated the *excess* form $\dot{x}^o = D w^o \max(0, 1 - 1/s)$ since the deadzone fix below, and `runtime/object_mjx.py` matches it, so both object plants moved and only the estimator stayed. $A^o$ and $A^r$ are then not the same physical quantity, and no $\rho$ can make the blocks agree. `"twist_exact"` inverts what the plant runs: $\dot{x}^o \parallel D w^o$ gives $\lVert\dot{x}^o\rVert = s - 1$, hence $w^o = D^{-1}(1 + \lVert\dot{x}^o\rVert)\,\dot{x}^o/\lVert\dot{x}^o\rVert$ — the magnitude sits at the cone whenever the block slides, which *is* quasi-static pushing (the twist fixes the direction, Coulomb fixes the magnitude). `"twist"` understates $\lvert w \rvert$ by $(1 + \lVert\dot{x}^o\rVert)/\lVert\dot{x}^o\rVert$ — measured at 200× on `open_table`, and 300× on the lab rig, where $\lvert A^r \rvert$ median 0.0006 N faced $\lvert z \rvert$ 0.18 N. Opt-in, not the default: flipping it moves every ADMM number in the paper while leaving flat MPPI untouched. Below `twist_stick_speed` the estimate ramps to zero, since a sticking block's twist direction is pure noise. |
