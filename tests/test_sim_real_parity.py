...
`examples/pusht/pusht_real.py` builds its own `PushT` + `ADMM` instead of
calling `oim.worlds.sim3d.build.build_admm_3d`, and that duplication has
silently diverged four times -- each caught only after hardware runs were
produced under it. The driver's own comments record them: the robot
rollout ran at 1 substep while sim read the config's, the object
sampler's noise/temperature were substituted, `rho_torque` was passed as
a bare scalar so torque was penalised 10x weaker, and `planning_iterations`
/`planning_ls_iterations` were never passed at all so every hardware run
solved contacts at the MJCF's 20/20 against sim's 40/30.

This pins the two constructions together: given the SAME config, they must
produce the same task and the same controller. A new knob added to one
builder and not the other fails here instead of on the robot.
...
def _args(mod):
    ...
    return types.SimpleNamespace(
        ...
        consensus=adm["consensus"], consensus_source=None,
        ...
        rho_torque=float(adm["rho_torque"]), rho_object=adm.get("rho_object"),
...
@pytest.mark.parametrize("attr", [
    "consensus", "consensus_source", "plant_form", "align_ref",
    "obstacle_form", "tip_z_form", "approach_mode",
])
...
def test_rho_vectors_match(built):
    """`rho_object` reaches the controller as `rho_object_scale`, the ratio
    the object block's penalty is multiplied by -- 1.0 when unset."""
    _, ctrl_r, _, ctrl_s, _ = built
    assert np.allclose(np.asarray(ctrl_r.rho_init, dtype=float),
                       np.asarray(ctrl_s.rho_init, dtype=float))
    assert np.allclose(np.asarray(ctrl_r.rho_object_scale, dtype=float),
                       np.asarray(ctrl_s.rho_object_scale, dtype=float))
...
MUST_MATCH = {
    "costs": [
        "obstacle_form",       # margin-gated barrier vs always-on exponential
        "approach_mode",       # SDF-wall distance vs object-origin distance
        "align_ref",           # object plan endpoint vs global goal
        "w_effort_normalized",  # wrench-limit normalized vs raw N / N.m
    ],
    "admm": [
        "plant_form",          # excess vs quasi-static limit surface
        "consensus",           # which variable the blocks agree on
        "consensus_source",    # how A^r is estimated
