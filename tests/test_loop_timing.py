...
def test_stamps_default_to_nan_without_an_interface_that_has_them() -> None:
    log = {}
    rr._log_read_stamps(log, _world(None))
    for key in rr._STAMP_READ_KEYS:
        assert len(log[key]) == 1 and np.isnan(log[key][0])
    rr._log_publish_stamps(log, {})
    assert np.isnan(log["ros_cmd_pub"][0]) and log["cmd_pub_index"] == [-1]
    assert rr._stamp_summary(log, 0) == ""
...
def test_stamps_align_and_summarise() -> None:
    ...
    for k in range(3):
        base = 10.0 + k
        rr._log_read_stamps(log, _world({
    ...
    rr._log_publish_stamps(log, pub)
    assert len(log["ros_cmd_pub"]) == 3 and log["cmd_pub_index"] == [0, 1, -1]
    line = rr._stamp_summary(log, 1)
    ...
    assert rr._stamp_summary(log, 2) == ""
    summary = rr._stamp_summary(log)
