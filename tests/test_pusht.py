...
def test_xarm6_tip_height_is_not_faded_but_tilt_now_is() -> None:
    ...
    # align=0 (weight zeroed), approach=0 (w_ee=0), and tilt is now faded
    # to 0 at the goal too -- whatever is left is exactly tip_height (plus
    # contact_z/joint3_cave, which read 0 at this pose).
    ...
    contact_z = task._contact_z_cost(state)
...
def test_xarm6_approach_fades_linearly() -> None:
    ...
    raw_approach = task.w_ee * max(d_ee, 0.0)  # r0=0, so clip is a no-op
...
def test_xarm6_effort_fades_near_goal() -> None:
    ...
    raw_effort = task.r_r * jnp.sum(control**2)
